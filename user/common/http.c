...
static void http_set_server_2(u8 * buf, u16 len)
{
    ...
    {
        snprintf(cmd, MAX_HTTPCMD_LEN, "AT+HTTPPARA=\"USERDATA\",\"Filename:%s\"" CR, http_localFileName);
        modem_AT((u8 *)cmd);
    }
    else if(strstr((const char *)buf, "OK") && strstr((const char *)buf, "USERDATA"))
    {
        ...
        else if(http_type == HTTP_UPLOAD)
        {
            ...
            snprintf(cmd, MAX_HTTPCMD_LEN, "AT+HTTPDATA=%d,100000" CR, data_len);
            modem_AT((u8 *)cmd);
        }
    }
    else
...
static void http_post_start(u8 * buf, u16 len)
{
    ...
    if(strstr((const char *)buf, "DOWNLOAD"))     //until all the data post
    {
        ...
        LOG_DEBUG("write data ok is poat_len = %d", file_offset);
